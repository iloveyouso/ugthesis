...
def process_images(hypes, savename, image_orig, detected, var, gt=None, out_res=None, threshold=None, printout=False):
    ...
    if savename is not None:
        ...
        fig.savefig(str(savename + '_detection.png')) #TODO
        plt.close(fig)
        
        ##################################
        # BJKIM: 512*512 png file returned.
        ##############################
        data = thresholded.numpy() # this is 512 * 512 array.
        fig, ax = plt.subplots()
        ax.imshow(data, cmap='gray')
        fig.savefig(str(savename + '_INPUT.png'))
        print(savename)
        
        #####################
        # OPTION 1: Below is the working_well code.
        # BJKIM: Find the center / radius of the circle.
        #######################
        # import cv2
        
        # # Convert the numpy array to an image
        # image = np.uint8(data * 255)  # Convert to 0-255 scale for image processing

        # # Find contours
        # contours, _ = cv2.findContours(image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        # # Assume the largest contour is the circle we need
        ...
        ############
        # OPTION 2: Below is the 도전 code. for multiple circles.
        ####
        ...
        for center, radius in circles:
            ...
            print("Center of the circle:", center)
            print("Radius of the circle:", radius)
            temp_list.extend(center)
            temp_list.append(radius)
            big_list.append(temp_list)
        
        big_list.insert(0, len(circles))
        print(big_list)
        ...
        plt.savefig(str(savename + '_circle.png'))

    return F_score, big_list
